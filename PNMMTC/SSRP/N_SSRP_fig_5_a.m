...
x0= zeros(n, 1);
tic;
dxdt1 = @(t, x,grad) -((c1/(t+w))/(norm(x-proximal(x - lambda * grad, lambda * alpha)))^(1-R_da1)+(c2/(t+w))/norm(x-proximal(x - lambda * grad, lambda * alpha))^(1-R_da2))*(x-proximal(x - lambda * grad, lambda * alpha))+N;
[t3, x3] = ode23(@(t, x) dxdt1(t, x,A'*(A * x - b)),tspan, x0);
time1= toc
fprintf('TvNOA ||x-x^*||^2 = %.4e\n',norm(x3(end,:)'-x)^2);
%%
%PNA
dxdt2 = @(t, x,grad) -40*(x-proximal(x - lambda * grad, lambda * alpha))+N;
tic;
[t1, x1] = ode23(@(t, x) dxdt2(t, x,A'*(A * x - b)),tspan, x0);
time2= toc
...
R_da2 = 1.5;
% 
dxdt3 = @(t, x,grad) -(A_da/(norm(x-proximal(x - lambda * grad, lambda * alpha)))^(1-R_da1)+B_da/norm(x-proximal(x - lambda * grad, lambda * alpha))^(1-R_da2)+C_da)*(x-proximal(x - lambda * grad, lambda * alpha))+N ;
tic;
[t2, x2] = ode23(@(t, x) dxdt3(t, x,A'*(A * x - b)),tspan, x0);
time3= toc
fprintf('FXPNA ||x-x^*||^2 = %.4e\n',norm(x2(end,:)'-x)^2);
%%
%PNMPTC
tspan = [0 1];
tic;%
[t4, x4] = ode23(@(t, x) PTDDN_dxdt(t, x,A,b,A'*(A * x - b),n,N),tspan, x0);
time4= toc
...
R_da1 = 0.5;
tspan = [0 5];
dxdt5 = @(t, x,grad) -(A_da/(norm(x-proximal(x - lambda * grad, lambda * alpha)))^(1-R_da1))*(x-proximal(x - lambda * grad, lambda * alpha)) +N ;
tic;
[t5, x5] = ode23(@(t, x) dxdt5(t, x,A'*(A * x - b)),tspan, x0);
time5= toc
...
x0= zeros(n, 1);
proximal = @(x_new, da) sign(x_new) .* max(abs(x_new) - da, 0);
dxdt6 = @(t, x,grad) -(Pp/Tp)*((c1/(t+w))/(norm(x-proximal(x - lambda * grad, lambda * alpha)))^(1-R_da1)+(c2/(t+w))/norm(x-proximal(x - lambda * grad, lambda * alpha))^(1-R_da2))*(x-proximal(x - lambda * grad, lambda * alpha))+N;
tic;
[t6, x6] = ode23(@(t, x) dxdt6(t, x,A'*(A * x - b)),tspan, x0);
time6= toc
...
%%
function x_t = PTDDN_dxdt(t,x,A,b,grad,n,N)
alpha = 0.001; 
lambda = 1.5; 
Tp = 1;
r = 1;
...
u = @(t) r/((Tp-t)^1.2);
    if t <Tp-1e-12
        ...
        x_t = -(1/(1e-12)^1.2) *(x-proximal(x - lambda * grad, lambda * alpha))+N;
    end
end
