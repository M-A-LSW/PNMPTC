...
x0= zeros(n, 1);
tic;
dxdt1 = @(t, x,grad) -((c1/(t+w))/(norm(x-proximal(x - lambda * grad, lambda * alpha)))^(1-R_da1)+(c2/(t+w))/norm(x-proximal(x - lambda * grad, lambda * alpha))^(1-R_da2))*(x-proximal(x - lambda * grad, lambda * alpha));
...
tspan = [0 1];
tic;%
[t4, x4] = ode23(@(t, x) PTDDN_dxdt(t, x,A,b,A'*(A * x - b),n),tspan, x0);
time4= toc
fprintf('FNMPTC ||x-x^*||^2 = %.4e\n',norm(x4(end,:)'-x)^2);
%%
%FNPGNN
A_da = 10;
R_da1 = 0.5;
...
x0= zeros(n, 1);
proximal = @(x_new, da) sign(x_new) .* max(abs(x_new) - da, 0);
dxdt6 = @(t, x,grad) -(Pp/Tp)*((c1/(t+w))/(norm(x-proximal(x - lambda * grad, lambda * alpha)))^(1-R_da1)+(c2/(t+w))/norm(x-proximal(x - lambda * grad, lambda * alpha))^(1-R_da2))*(x-proximal(x - lambda * grad, lambda * alpha));
tic;
...
hold off;
%%
function x_t = PTDDN_dxdt(t,x,A,b,grad,n)
alpha = 0.001; 
lambda = 1.5; 
Tp = 1;
r = 1;
