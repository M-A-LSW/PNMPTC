...
tic;
dxdt1 = @(t, x,grad) -((c1/(t^2+w))/(norm(x-proximal(x - lambda * grad, lambda * alpha)))^(1-R_da1)+(c2/(t^2+w))/norm(x-proximal(x - lambda * grad, lambda * alpha))^(1-R_da2))*(x-proximal(x - lambda * grad, lambda * alpha))+N;
...
tic
tspan = [0 1];
[t4, x4] = ode23(@(t, x) PTDDN_dxdt(t, x,-(1/m)*X_with_int' * (y - exp(X_with_int * x)),N),tspan, x0);
time4 = toc
...
tspan = [0 1];
proximal = @(x_new, da) sign(x_new) .* max(abs(x_new) - da, 0);
dxdt6 = @(t, x,grad) -(Pp/Tp)*((c1/(t^2+w))/(norm(x-proximal(x - lambda * grad, lambda * alpha)))^(1-R_da1)+(c2/(t^2+w))/norm(x-proximal(x - lambda * grad, lambda * alpha))^(1-R_da2))*(x-proximal(x - lambda * grad, lambda * alpha))+N;
tic;
