...
figure;
omega = [0.5, 1, 1.5, 2];
plot3(t1, omega(1)*ones(size(t1)),cc1,  'Color', [0.90 0.29 0.23], 'Marker', 'v', 'LineStyle', '-', 'MarkerSize', 4);
hold on;
plot3(t2, omega(2)*ones(size(t2)),cc2, 'Color', [0.27 0.42 0.81], 'Marker', 'h', 'LineStyle', '-', 'MarkerSize', 4);
plot3(t3, omega(3)*ones(size(t3)),cc3,  'Color', [0.96 0.73 0.12], 'Marker', 'p', 'LineStyle', '-', 'MarkerSize', 4);
plot3(t4, omega(4)*ones(size(t4)),cc4,  'Color', [0.57 0.27 0.67], 'Marker', 'o', 'LineStyle', '-', 'MarkerSize', 4);
hold off;
xlabel('t');
ylabel('\omega');
zlabel('log_{10}||x-x^*||^2');
legend('\omega=0.5','\omega=1','\omega=1.5','\omega=2', 'Location','best');
grid on;
view(45, 20); 
set(gca, 'YTick', omega);
%%
